...
var (
	specFS           fs.FS = os.DirFS("../data/loader")
	multiFileSpecFS  fs.FS = os.DirFS("../data/loader-multiple-files")
	xTagGroupsSpecFS fs.FS = os.DirFS("../data/xTagGroups")
)

func Test_LoadFS(t *testing.T) {
	spec, err := loader.LoadWithNameFS(specFS, "pet-store.yml")
...
func Test_LoadFS_JsonFile(t *testing.T) {
	spec, err := loader.LoadWithNameFS(specFS, "pet-store.json")

	require.NoError(t, err)
	require.NotNil(t, spec)
}

func Test_LoadFS_InvalidFileType(t *testing.T) {
	spec, err := loader.LoadWithNameFS(specFS, "pet-store.xyz")

	require.ErrorContains(
		t,
		err,
		`file 'pet-store.xyz' is not a YAML or JSON file, supported extensions are [yml|yaml|json]`,
	)
	require.Nil(t, spec)
}

func Test_LoadFS_MultipleFiles(t *testing.T) {
	spec, err := loader.LoadWithNameFS(multiFileSpecFS, "api.yml")
...
func Test_LoadedFileShouldHaveIdenticalContentFS(t *testing.T) {
	specFromMultipleFiles, err := loader.LoadWithNameFS(multiFileSpecFS, "api.yml")
	require.NoError(t, err)

	specFromSingleFile, err := loader.LoadWithNameFS(specFS, "pet-store.yml")
	require.NoError(t, err)

	require.True(t, reflect.DeepEqual(specFromMultipleFiles, specFromSingleFile))
}

func Test_LoadFS_DocumentedPath(t *testing.T) {
	spec, err := loader.LoadWithNameFS(specFS, "pet-store.yml")
	require.NoError(t, err)
...
func Test_LoadFS_XTagGroups(t *testing.T) {
	spec, err := loader.LoadWithNameFS(xTagGroupsSpecFS, "withXTagGroups.yaml")
	require.NoError(t, err)
...
// 	require.Equal(
// 		t,
// 		model.Tag{Name: "pets", Description: "Everything about your Pets"},
// 		spec.Tags[0],
// 	)
...
// 	require.Equal(
// 		t,
// 		model.GenericObject{"format": "int64", "type": "integer"},
// 		petSchema["properties"].(model.GenericObject)["id"],
// 	)
// 	require.Equal(
// 		t,
// 		model.GenericObject{"type": "string"},
// 		petSchema["properties"].(model.GenericObject)["name"],
// 	)
// 	require.Equal(
// 		t,
// 		model.GenericObject{"type": "string"},
// 		petSchema["properties"].(model.GenericObject)["tag"],
// 	)
...
// 	require.Equal(
// 		t,
// 		model.GenericObject{"type": "integer", "format": "int32"},
// 		errorSchema["properties"].(model.GenericObject)["code"],
// 	)
// 	require.Equal(
// 		t,
// 		model.GenericObject{"type": "string"},
// 		errorSchema["properties"].(model.GenericObject)["message"],
// 	)
// }
